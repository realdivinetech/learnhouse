...
function LinkToUserGroup(props: LinkToUserGroupProps) {
    ...
    const handleLink = async () => {
        const res = await linkResourcesToUserGroup(selectedUserGroup, courseStructure.course_uuid, org.id, access_token)
        if (res.status === 200) {
    ...
    return (
        <div className='flex flex-col space-y-1 '>
            <div className='flex bg-yellow-100 text-yellow-900 mx-auto w-fit mt-3 px-4 py-2 space-x-2 text-sm rounded-full items-center'>
                <Info size={19} />
                <h1 className=' font-medium'>{t('dashboard.courses.access.usergroups.modals.warning')}</h1>
            </div>
            <div className='p-4 flex-row flex justify-between items-center'>
                {usergroups?.length >= 1 &&
                    <div className='py-1'>
                        <span className='px-3 text-gray-400 font-bold rounded-full py-1 bg-gray-100 mx-3'>{t('dashboard.courses.access.usergroups.modals.usergroup_name')} </span>
